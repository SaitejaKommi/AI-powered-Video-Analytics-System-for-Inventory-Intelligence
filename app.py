...
@st.cache_resource
def init_pipeline():
    logger.info("Initializing heavy CV pipeline engines...")
    detector = YOLODetector(model_path, allowed_classes, class_names, conf_threshold)
    tracker = ObjectTracker(max_age=track_max_age, n_init=track_n_init)
    counter = ZoneCounter(zones_config, counting_class, db_path)
    anomaly_engine = AnomalyDetector(counting_class, missing_tolerance, db_path)
    return detector, tracker, counter, anomaly_engine

detector, tracker, counter, anomaly_engine = init_pipeline()
    ...
    try:
        ...
        while st.session_state.stream_active:
            ...
            if frame_count % 3 == 0:  # Mild throttle for render stability
                ...
                frame_annotated = draw_zones_and_count(frame_annotated, zones_config, current_count, counting_class)
